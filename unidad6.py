...
def send_confirmation(email_usuario, nombre_usuario):
    ...
    cuerpo = f"Hola {nombre_usuario},\n\nHemos recibido tu documento. Gracias por enviarlo. En los próximos días te escribiremos.\n\nAtentamente,\nUnidad de Revisión de Artículos Científicos"
...
def send_files_to_admin(file_data, file_name):
    mensaje = MIMEMultipart()
    mensaje['From'] = EMAIL_USER
    mensaje['To'] = NOTIFICATION_EMAIL
    mensaje['Subject'] = "Nuevo documento recibido"

    cuerpo = "Se ha recibido un nuevo documento adjunto y el registro de transacciones."
    mensaje.attach(MIMEText(cuerpo, 'plain'))
...
if st.button("Enviar archivo"):
    if not nombre_completo:
        ...
        with st.spinner("Enviando archivo, por favor espera..."):
            ...
            log_transaction(nombre_completo, email, file_name)

            # Enviar correos
            send_confirmation(email, nombre_completo)
            send_files_to_admin(file_data, file_name)

            st.success("Archivo subido y correos enviados exitosamente.")
            st.success("Gracias por usar este servicio, en breve recibirás una notificación en tu correo electrónico. Ya puedes cerrar esta aplicación.")
